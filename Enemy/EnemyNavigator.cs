...
public class EnemyNavigator : MonoBehaviour
{
    ...
    void ReattachAfterAirOrLandingLock()
    {
        ...
        {
            float heightDelta = Mathf.Abs(agent.nextPosition.y - transform.position.y);
            float planarDelta = new Vector2(agent.nextPosition.x - transform.position.x, agent.nextPosition.z - transform.position.z).sqrMagnitude;
            if (heightDelta > hardSnapHeightDistance || planarDelta > hardSnapDistance * hardSnapDistance)
                ReattachAgentToClosestNavMesh(transform.position);
        }
